...
authRouer.route('/password-reset-request')
...
authRouer.route('/password-reset-request/reset-password')
  .post(function(req, res) {
    PasswordResetRequest.findOne({
      where: {
        uuid: req.body.uuid
      ...
      if (!passwordResetRequest) {
        return res.status(404).json({
          error: 'No password reset request'
        })
      }
...
authRouer.route('/forgot-password')
        ...
        if (req.headers.authorization) {
          ...
          User.findOne({
            ...
            if (existingUser) {
              return res.json({
                user: existingUser,
                authToken: createToken(existingUser)
              });
            }
            var user = User.build();
            user.google = profile.sub;
